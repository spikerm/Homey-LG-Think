...
class LGWasherDevice extends Homey.Device {
  ...
  async refreshThinQ2() {
    ...
    if (errorNow !== 'ERROR_NO' && errorNow !== this._lastError) {
      await this._errorTrigger.trigger(this, { error: this._lastError || 'Onbekende fout', message: `LG wasmachine storing: ${this._lastError || 'Onbekende fout'}.` }).catch(this.error);
    }
  ...
  async wakeupWasher() {
    ...
    await this._sleep(1500);
    await this.refreshNow().catch(err => this.log(`Wake refresh: ${err.message || err}`));
    return response;
  }
