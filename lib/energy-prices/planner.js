...
function normalizeSlots(slots, source) {
  return (slots || []).map(p => {
    ...
    return { ...p, start, end: Number.isFinite(end) ? end : start + SLOT_MS, price, source: p?.source || source };
...
function daysForWindow(from, until) {
  ...
  const days = [];
  for (let day = first; day.toMillis() <= last.toMillis() && days.length < 4; day = day.plus({ days: 1 })) days.push(day.toISODate());
  return days;
}
...
async function slimLadenSlots(app) {
  ...
  const mapping = new Map([['today', now.toISODate()], ['tomorrow', now.plus({ days: 1 }).toISODate()]]);
  const slots = raw.map(row => {
    ...
    return { start: start.toMillis(), end: start.plus({ minutes: 15 }).toMillis(), price, priceField: 'slimladen_all_in', source: 'slimladen' };
  }).filter(Boolean);
...
function assertComplete(slots, from, until, provider) {
  const useful = normalizeSlots(slots, provider).filter(p => p.end > from && p.start < until).sort((a, b) => a.start - b.start);
...
async function getCompleteWindow(app, from, until) {
  ...
  let order;
  if (configured === 'auto') {
    const hasTibber = Boolean(String(setting(app, 'tibber_api_token', '')).trim());
    order = hasTibber ? ['tibber', 'slimladen', 'homey-energy'] : ['homey-energy', 'slimladen'];
    app.log(`Prijsprovider Auto volgorde: ${order.join(' -> ')}.`);
...
function averagePriceForWindow(slots, startMs, endMs) {
  ...
  let cursor = start, weighted = 0, covered = 0;
  for (const slot of slots) {
    if (slot.end <= cursor) continue;
    if (slot.start >= end || slot.start > cursor + 1000) break;
    const a = Math.max(cursor, slot.start), b = Math.min(end, slot.end);
    if (b > a) { const ms = b - a; weighted += Number(slot.price) * ms; covered += ms; cursor = b; }
    if (cursor >= end) break;
  }
...
async function calculate(app, { earliestMs, deadlineMs, durationMinutes }) {
  ...
  if (!Number.isFinite(deadline) || deadline <= earliest + durationMs) throw new Error('De eindtijd ligt te vroeg voor de gekozen programmaduur en minimale startmarge.');
  ...
  for (const start of [...new Set(starts)]) {
    ...
    if (Number.isFinite(averagePrice)) candidates.push({ start, end, averagePrice, durationMinutes: Math.round(duration) });
  }
  ...
  return { provider, earliestStart: earliest, best: candidates[0], alternatives: candidates.slice(1, 5), slots: slots.map(s => ({ start: s.start, end: s.end, price: s.price, source: s.source || provider, priceField: s.priceField || null })) };
}

module.exports = { MIN_START_AHEAD_MS, minimumPlanningStart, averagePriceForWindow, getCompleteWindow, calculate };
