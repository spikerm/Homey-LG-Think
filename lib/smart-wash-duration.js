...
async function ensureInsightsCapabilities(device) {
  // The driver already calls this for every paired washer shortly after startup,
  // making it a safe central hook for the verified wake/start wrapper as well.
...
function applyLearnedDurations(device, state) {
  ...
  const programs = state.programs.map(program => {
    ...
    return { ...program, expectedDurationMinutes: learned.averageMinutes, durationLearning: learned };
  });
...
async function finaliseSession(device, session, elapsedMinutes) {
  if (!session?.programId || !Number.isFinite(elapsedMinutes) || elapsedMinutes < 1 || elapsedMinutes > 600) return;
  const history = getHistory(device);
  const current = history[session.programId];
  ...
  if (!matched) return;
  history[session.programId] = { ...current, samples, lastActualMinutes: elapsedMinutes };
...
async function recordFromLive(device, live = null) {
  ...
  history[programId] = { samples, count: valid.length, averageMinutes, lastMinutes: Math.round(minutes), lastAt: Date.now() };
  await device.setStoreValue(STORE_KEY, history);
